...
import { deleteItem } from "../utilities/script";
...
const CartItem = ({ cartItem, totalPrice }) => {
  ...
  const handleFavourites = (id) => {
    ...
    const isExist = favItems.find((favItem) => favItem.id == id);
    if (isExist) {
      toast.error("Already Exist in favoruites");
      return;
    } else {
      toast.success("Successfully added to your favourites");
    }
    const singleFavourite = data.find((favItem) => favItem.id == id);
    favItems.push(singleFavourite);
